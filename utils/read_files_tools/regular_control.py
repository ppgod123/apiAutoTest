...
class Context:
    ...
    @classmethod
    def time_after(cls, time_gap: int):
        """获取X天后的当前时间"""

        _time_after_week = (date.today() + timedelta(days=+time_gap)).strftime("%Y-%m-%d  %H:%M:%S")
    ...
    @classmethod
    def time_after_zero(cls, time_gap: int):
        """获取X天后0点整的时间"""

        _time_after_zero = (date.today() + timedelta(days=+time_gap)).strftime("%Y-%m-%d") + " 00:00:00"
        return _time_after_zero
